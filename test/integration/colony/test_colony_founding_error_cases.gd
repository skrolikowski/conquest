...
func test_found_colony_on_water_fails() -> void:
	...
	var water_tile : Vector2i = Vector2i(4, 4)
	...
	var result: ColonyFoundingWorkflow.Result = colony_manager.found_colony(water_tile, world_pos, settler)
	...
	assert_eq(colony_manager.colonies.size(), 0, "Should have 0 colonies")
...
func test_cancel_found_colony_twice_fails_gracefully() -> void:
	...
	var result_1: ColonyFoundingWorkflow.Result = colony_manager.cancel_found_colony()
	await wait_physics_frames(1)

	assert_true(result_1.is_ok(), "First cancel should succeed")

	# Act - cancel again (should fail)
	var result_2: ColonyFoundingWorkflow.Result = colony_manager.cancel_found_colony()
	await wait_physics_frames(1)
...
func test_found_colony_with_null_settler_fails() -> void:
	...
	var result: ColonyFoundingWorkflow.Result = colony_manager.found_colony(tile_pos, world_pos, null)
	await wait_physics_frames(1)
...
func test_undo_create_colony_on_active_building_fails() -> void:
	...
	assert_eq(colony_manager.colonies.size(), 1, "Colony should still exist")
